import { Plus, DoorOpen, Pencil } from 'lucide-react'
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-300">
      <div className="flex justify-between items-center border-b border-slate-200 pb-6">
        ...
        <Button onClick={onOpenCreateRoom} className="bg-purple-600 hover:bg-purple-700 shadow-md text-white cursor-pointer">
      ...
      {rooms.length === 0 && page === 1 ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {rooms.map(room => (
              <Card key={room.id} className="p-6 bg-white/80 border-slate-200/60 shadow-sm hover:shadow-md transition-shadow group relative cursor-pointer">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center">
                    <Button variant="ghost" size="icon" onClick={() => onEditRoom(room)} className="h-8 w-8 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-full transition-colors cursor-pointer">
