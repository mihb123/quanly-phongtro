...
func NewTenantHandler(service tenantsvc.TenantService) *TenantHandler {
...
func (h *TenantHandler) RegisterTenant(w http.ResponseWriter, r *http.Request) {
	...
	httpx.WriteJSON(w, http.StatusCreated, newTenantResponse(*tenant), "")
...
func (h *TenantHandler) UpdateTenantInfo(w http.ResponseWriter, r *http.Request) {
	...
	in := tenantsvc.UpdateTenantInput{}
	...
	if err != nil {
		...
		case errors.Is(err, model.ErrUnauthorized):
			...
			httpx.WriteError(w, http.StatusForbidden, "forbidden: you do not manage this tenant")
		case errors.Is(err, model.ErrPhoneAlreadyExists):
			logger.Warn(r, http.StatusBadRequest, "phone already exists", err)
			httpx.WriteError(w, http.StatusBadRequest, err.Error())
		default:
	...
	httpx.WriteJSON(w, http.StatusOK, newTenantResponse(*updated), "tenant updated successfully")
}
