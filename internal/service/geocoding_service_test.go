...
func TestGeocodingService_GoogleMaps(t *testing.T) {
	...
	t.Run("client get error", func(t *testing.T) {
		// use an invalid url format to trigger Get error
		old := googleGeocodeBaseURL
		googleGeocodeBaseURL = "http://127.0.0.1:0" // this should fail to connect, or use bad scheme
		svc := NewGeocodingService("key")
		// override client transport to fail always
		svc.(*geocodingServiceImpl).client.Transport = &errorTransport{}
		_, err := svc.ReverseGeocode(21.0, 105.8)
		assert.Error(t, err)
...
func TestGeocodingService_OSM(t *testing.T) {
	...
	t.Run("client do error", func(t *testing.T) {
		...
		svc.(*geocodingServiceImpl).client.Transport = &errorTransport{}
		_, err := svc.ReverseGeocode(5.0, 0.0)
		assert.Error(t, err)
	...
	t.Run("new request error", func(t *testing.T) {
		old := osmGeocodeBaseURL
		osmGeocodeBaseURL = "::invalid::" // invalid URL triggers NewRequest error
		_, err := svc.ReverseGeocode(6.0, 0.0)
		assert.Error(t, err)
		osmGeocodeBaseURL = old
	})
}
